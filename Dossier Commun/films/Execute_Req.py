...
def TakeDate(_input):
    while not (IsValidDate(_input) or len(_input) == 0 or _input.lower()=="q") :    
        _input = input("Veuillez entrer une date valide (pressez \"Q\" ou \"entrée\" pour quitter ) : ")
    else: 
...
def Execute_Req( _req, _header = 0, _headerToPrint = 0):
    _matchIdToNumber = {}
    try: 
        connection = MC.connect(host = 'localhost', database='eqlaflix', user = 'root', password = 'isaac' )
        ...
        print("une erreur s'est produite dans le script principt principal")
    finally: 
...
def ChooseInTable(_filmToSave): 
    ...
    _userInput = input("\n votre choix : ")
    connexion = MC.connect(host = 'localhost', database='eqlaflix', user = 'root', password = 'isaac' )
