...
# menu de sélection
filmHeader = [ "Tout sélectionner", "ID", "TITRE DU JEU VIDEO", "DATE DE SORTIE", "DEVELOPPEUR", "EDITEUR", "CATEGORIE"]

# entete séléctionner par le user à afficher avec les données
# filmHeaderToPrint = [1, 2, 3, 4, 5, 6, 7, 8]


sqlFilm = {1:"IDVieogames", 2:"Videogames_Titre", 3:"Videogames_DateSortie", 4:"Videogames_Développeur", 5:"Videogames_Editeur", 6:"Videogames_Categorie"}


defaultHeaderList = [1, 2, 3, 4, 5, 6]

# les items à modifier lors de l'ajout de films
filmHeader2 = ["Videogames_Titre", "Videogames_Développeur", "Videogames_Editeur", "Videogames_Categorie", "Videogames_DateSortie" ]




def FilmMenuSentence(_filmHeaderToPrint):
    _fimMenuSentence = "Bienvenu cHEZ EqlaFlix/JEUX. \n Veuillez choisir les différentes options d'affichage : "
...
def Execute_ReqRandom( _req, _header = 0, _headerToPrint = 0):
    _matchIdToNumber = {}
    randNum = TakePositifNumber(input("Combien de JEUX aléatoires souhaitez vous générer ? "))
    connect = AllUsersConnection()
    Cursor = connect.cursor() 
    req = "select count(*) from Videogames;"
    Cursor.execute(req)
    ...
    try: 
        ...
        if 0 < randNum <= maxNum : 
            ...
            print("Vous avez choisi de ne pas afficher de jeux. ")
            return
        else: 
            print(f"Nous ne disposons que de {maxNum} jeux dans notre base de données.")
...
def ChooseInTable(_filmToSave): 
    ...
    _header =  [ "Tout sélectionner", "ID", "TITRE DU JEU VIDEO", "DATE DE SORTIE", "DEVELOPPEUR", "EDITEUR", "CATEGORIE"]
    _headerToPrint = [2]
